...
function KitchenInner() {
  ...
  const businessType = user
    ? ((user as any).businessType as string | undefined)
    : undefined;
  ...
  async function load() {
    ...
    setError(null);
    try {
      const data = await apiClient.get<KitchenOrder[]>("/kitchen/orders", {
        token,
      });
      setItems(data ?? []);
    } catch (e) {
  ...
  async function patch(action: "start" | "ready" | "complete", id: string) {
    ...
    setBusyId(id);
    try {
      await apiClient.patch(`/kitchen/orders/${id}/${action}`, undefined, {
        token,
      });
      await load();
    } catch (e) {
              ...
              {items.map((o) => (
                <tr key={o.id}>
                  ...
                  <td style={{ padding: 8, borderBottom: "1px solid #f0f0f0" }}>
                    ...
                    >
                      <Can
                        permission={PERMISSIONS.KITCHEN_UPDATE}
                        fallback={null}
                      >
                        {businessType === "CAFE" ? (
                          <>
