...
function InventoryHistoryInner() {
  ...
  useEffect(() => {
    async function run() {
      ...
      setError(null);
      try {
        const data = await apiClient.get<InventoryHistoryResponse>('/inventory/history', {
          token,
        });
        setRows(data ?? []);
      } catch (e) {
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.id}>
              ...
              <td style={{ padding: 8, borderBottom: '1px solid #f0f0f0' }}>{r.quantity ?? '-'}</td>
              <td style={{ padding: 8, borderBottom: '1px solid #f0f0f0' }}>{r.inventoryItemId ?? '-'}</td>
